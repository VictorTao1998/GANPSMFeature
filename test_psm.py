...
def train_sample(sample, psmnet_model, psmnet_optimizer, isTrain=True):
    ...
    img_L = F.interpolate(img_L, scale_factor=0.5, mode='nearest',
                             recompute_scale_factor=False)  # [bs, 1, H, W]
    img_R = F.interpolate(img_R, scale_factor=0.5, mode='nearest',
                             recompute_scale_factor=False)  # [bs, 1, H, W]
...
if __name__ == '__main__':
    ...
    val_dataset = MessytableDataset(cfg.REAL.TRAIN, gaussian_blur=False, color_jitter=False, debug=args.debug, sub=100, isReal=True)
    if is_distributed:
